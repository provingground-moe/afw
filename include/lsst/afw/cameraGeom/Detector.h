...
public:
    ...
    /** Return the purpose type of this detector. */
...
protected:
    ...
    struct Fields {
        std::string name = "";                      //< name of detector's location in the camera
        int id = 0;                                 //< detector numeric ID
        DetectorType type = DetectorType::SCIENCE;  //< type (actually purpose) of detector
        std::string serial = "";                    //< serial "number" that identifies the physical detector
        lsst::geom::Box2I bbox;                     //< bounding box
        Orientation orientation;                    //< position and orientation of detector in focal plane
        lsst::geom::Extent2D pixelSize;             //< pixel size (mm)
        CrosstalkMatrix crosstalk;                  //< crosstalk coefficients
        std::string physicalType;                   //< physical type of detector, e.g. "ITL", "HgCdTe"
    };
 ...
 * Amplifier objects.
 *
 * Detector is immutable, and can only be modified via one of its Builder
...
protected:
    ...
    /// Implementation for DetectorBase::getFields.
 ...
 * and hence Detector::Builder cannot inherit directly from Detector.  But in
 * both Python and templated C++ code, the container interfaces of Detector
 * and Detector::Builder are identical (i.e. they're "duck type" equivalent),
 * aside from the fact that Detector::Builder also permits addition and removal
 * of amplifiers.
 */
class Detector::Builder : public DetectorBase {
public:

    // Builder is base class with state, so we need to disable moves and
    // copies to avoid slicing.  As it turns out, its subclasses want to be
    // non-copyable and non-movable anyway.
     ...
     * constructed.
     *
     * Setting with an empty metrix is equivalent to calling
     * `unsetCrosstalk()`.
     */
    ...
    // The only reason we don't have more container APIs (for single-element
    // removal, insertion at arbitrary locations, etc) is that there's no
    // clear need for them right now, and we don't want to maintain more than
    // we need.  If we do identify a need for more APIs, we should add them.
 ...
 * new Detector, the original Camera is never updated.  PartialRebuilder it
 ...
 * all Detector::InCameraBuilder instances are always owned by a
 * Camera::Builder.
 *
...
private:
    ...
    // Transforms and coordinate systems that are specific to this detector or
    // not specific to any detector.  This does not include the
    // FOCAL_PLANE<->PIXELS connection, as that's derived from the orientation
    // and pixel size.
